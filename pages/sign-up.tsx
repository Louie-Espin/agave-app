...
const SignUpPage: NextPage<SignUpPageProps> = () => {
    ...
    const handleFormSubmit = async (values: { displayName: string, email: string, password: string, re_password: string }) => {
        try {
    ...
    return(
        <Wrapper elevation={0} passwordVisibility={passwordVisibility}>
            <form onSubmit={handleSubmit}>
                <H2 textAlign="center" mb={1}>Create an Account</H2>
                ...
                <BazarTextField mb={1.5} fullWidth size="small" variant="outlined"
                    name="displayName" label="Full Name" placeholder="Your Name"
                    onBlur={handleBlur} value={values.displayName} onChange={handleChange}
                    error={!!touched.displayName && !!errors.displayName} helperText={touched.displayName && errors.displayName}
                />

                <BazarTextField mb={1.5} fullWidth size="small" variant="outlined"
                    name="email" type="email" label="Email" placeholder="example@mail.com"
                ...
                <BazarTextField mb={2} fullWidth size="small" variant="outlined" name="re_password" label="Confirm Password"
                    autoComplete="on" onBlur={handleBlur} onChange={handleChange} value={values.re_password}
                    placeholder="*********" type={passwordVisibility ? "text" : "password"}
                    error={!!touched.re_password && !!errors.re_password} helperText={touched.re_password && errors.re_password}
